...
from DQNwithNoisyNet.prioritized_memory import Memory, WeightedMSE
import torch
import math
import random
...
'''class DeepQL(DQN_NoisyNet.DeepQL):
    def __init__(self, *args, lambda1=1.0, lambda2=1.0, lambda3=1e-5, n_step=3, **kwargs, ):
    ...
    def storeDemoTransition(self, s, a, r, s_, done, demoEpisode):
        ...
        episodeReplay = self.demoReplay[demoEpisode]  # replay of certian demo episode
        index = len(episodeReplay)
        data = (s, a, r, s_, done, (demoEpisode, index))
        episodeReplay.append(data)
        self.store(data, error + self.ed)

    def storeTransition(self, s, a, r, s_, done):
        s = torch.Tensor(s)
        s_ = torch.Tensor(s_)
        error = self.calcError((s, a, r, s_, done))
        self.store((s, a, r, s_, done, None), error + self.ea)
    ...
    def Jn(self, samples):
        ...
        for s, a, r, s_, done, isdemo in samples:
            ...
            ns, na, ns_, ndone = ns[-1], na[-1], ns_[-1], ndone[-1]
            discountedR = reduce(lambda x, y: (x[0] + self.gamma**x[1] * y, x[1] + 1), nr, (0, 0))[0]
            maxA = torch.Tensor(self.findMaxA(ns_))
            target = discountedR if ndone else r + self.gamma**self.n_step * self.net2(ns_, maxA)[0]
            predict = self.net(s,torch.Tensor(a))[0]
            loss += (target-predict)**2
        return loss / count
...
class DeepQLv2(DQN_NoisyNet.DeepQLv2):
    ...
    def storeDemoTransition(self, s, a, r, s_, done):
        s = torch.Tensor(s)
        s_ = torch.Tensor(s_)
        error = self.calcError((s, a, r, s_, done))
        self.store((s, a, r, s_, done, True), error + self.ed)

    def storeTransition(self, s, a, r, s_, done):
        s = torch.Tensor(s)
        s_ = torch.Tensor(s_)
        error = self.calcError((s, a, r, s_, done))
        self.store((s, a, r, s_, done, False), error + self.ea)

    def JE(self, samples):
        count = 0  # number of demo data
        loss = torch.tensor(0.0)
        for s, a, *_, isdemo in samples:
            if not isdemo:
                continue
